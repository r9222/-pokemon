...
function searchMoveInfo(userText) {
    ...
    for (const poke of POKE_DB) {
        ...
        for (let i = 0; i < lines.length; i++) {
            let l = lines[i];
            if (l.includes("レベルアップで覚える")) currentMethod = "レベルアップ";
            else if (l.includes("わざマシン・ひでんマシン")) currentMethod = "わざマシン・ひでんマシン";
            else if (l.includes("タマゴわざ")) currentMethod = "タマゴわざ";
            else if (l.includes("教えてもらえる")) currentMethod = "教えてもらえるわざ";

            if (l === targetMove && currentMethod !== "") {
                if (i + 5 < lines.length) {
                    ...
                    if (typesList.includes(type)) {
                        ...
                        if (!learningMethods[currentMethod].includes(poke.name)) {
                            learningMethods[currentMethod].push(poke.name);
                        }
                    }
                }
            }
    ...
    if (moveData) {
        let text = `【技データ】\n技名: ${moveData.name}\nタイプ: ${moveData.type}\n威力: ${moveData.power}\n命中: ${moveData.acc}\nPP: ${moveData.pp}\n効果: ${moveData.effect}\n\n`;
        text += `【この技を覚える全てのポケモン（初代限定）】\n`;
        for (const [method, pokes] of Object.entries(learningMethods)) {
            if (pokes.length > 0) {
                text += `[${method}] ${pokes.join("、")}\n`;
            }
        }
        return text.trim();
    }
